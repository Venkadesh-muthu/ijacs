<div class="container py-5">
  ...
  <?php if (!empty($groupedIssues)): ?>
    <?php foreach ($groupedIssues as $issueData): ?>
      <?php $issue = $issueData['issue']; ?>
      <?php $articlesGroup = $issueData['articles']; ?>
      <?php $serial = 1; ?>
      ...
      <div class="bg-warning text-dark rounded-1 p-4 p-md-5 mb-5 shadow-lg">
        <div class="row align-items-center g-4">
          <div class="col-12 col-md-3 text-center text-md-start">
            <img
              src="<?= base_url(!empty($issue['issue_image']) ? 'uploads/issues/' . $issue['issue_image'] : 'assets/images/no-image.png') ?>"
              alt="Cover" class="rounded shadow-sm img-fluid"
              style="max-width: 140px; max-height: 180px; object-fit: cover;">
          </div>

          <div class="col-12 col-md-9">
            <h5 class="mb-2"><?= date('F, Y', strtotime($issue['published_date'])) ?> (Special Issue)</h5>
            <h4 class="fw-bold mb-3">
              Volume: <?= esc($issue['volume_no']) ?>, Issue: <?= esc($issue['issue_no']) ?>
            </h4>
            <p class="mb-3 fs-6">
              📅 Published on:
        ...
        <div class="card mb-5 border-0 shadow-sm rounded-1 p-4">
          <div class="row g-4 align-items-center">
            <div class="col-md-9">
              <div class="mb-3">
                <?= $serial++ ?>.
                <span class="badge bg-dark me-2"><?= date('j M, Y', strtotime($article['published_date'])) ?></span>
